...
export const boardEndpoints = (builder: EndpointBuilder<BaseQueryFn, string, string>) => ({
     ...
     getBoardNotes: builder.query<{ id: string; board_id: string; content: never; created_at: string; updated_at: string } | null, string>({
          async queryFn(boardId) {
               try {
     ...
     saveBoardNotes: builder.mutation<{ success: boolean }, { boardId: string; content: never }>({
