...
const ProfilePage = () => {
     ...
     // Boards
     const { data: boards = [], isLoading: boardsLoading } = useGetMyBoardsQuery(currentUser?.id ?? '', { skip: !currentUser?.id });
               ...
               >
                    <div className="bg-slate-900 border border-slate-700/50 rounded-2xl shadow-xl p-8 flex flex-col gap-6">
                         ...
                         <div className="flex gap-5 items-center">
                              <Avatar src={session?.user?.image || ''} alt="Avatar" size={64} className="ring-2 ring-blue-400/40" />
                              <div>
                                   <div className="text-xl font-bold text-white">{session?.user?.name || 'User'}</div>
                         ...
                         {/* Boards */}
                         <div>
                              <h2 className="text-lg font-semibold text-slate-200 mb-2">My Boards</h2>
                              {boardsLoading ? (
                                   ...
                                   <ul className="space-y-2">
                                        {boards.map((b: { id: string; title: string; description?: string }) => (
                                             <li key={b.id}>
                                                  ...
                                                  {b.description && <div className="ml-10 text-xs text-slate-400">{b.description}</div>}
                                             </li>
                                        ))}
                                   </ul>
                              )}
