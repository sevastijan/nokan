     ...
     return (
          <div className="w-48 bg-slate-800/95 border border-slate-700/50 rounded-lg shadow-xl z-50 p-3 text-sm text-slate-200">
               ...
               <label className="flex items-center mb-2 space-x-2 hover:bg-slate-700/30 px-2 py-1 rounded cursor-pointer">
                    <input type="checkbox" checked={hasTasksOnly} onChange={(e) => setHasTasksOnly(e.target.checked)} className="form-checkbox text-blue-500 focus:ring-blue-400" />
                    <span>{t('dashboard.hasTasks')}</span>
               </label>
               <label className="flex items-center mb-2 space-x-2 hover:bg-slate-700/30 px-2 py-1 rounded cursor-pointer">
                    <input type="checkbox" checked={hasMembersOnly} onChange={(e) => setHasMembersOnly(e.target.checked)} className="form-checkbox text-blue-500 focus:ring-blue-400" />
