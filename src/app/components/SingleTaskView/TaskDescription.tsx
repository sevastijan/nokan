...
import { $getRoot, $getSelection, EditorState, LexicalEditor, $createParagraphNode, DecoratorNode, LexicalNode, SerializedLexicalNode, Spread } from 'lexical';
          ...
          return {
               img: () => ({
                    conversion: (element: HTMLElement) => {
                         ...
                         console.log('🖼️  Importing image node:', { src: src.substring(0, 50), alt });
                         return {
...
function ToolbarPlugin({ taskId, uploading, onUploadStart, onUploadEnd }: ToolbarPluginProps) {
     ...
     const handleImageUpload = async (file: File) => {
          ...
          try {
               ...
               if (!image?.signed_url) throw new Error('Nie udało się uzyskać URL obrazu');

               editor.update(() => {
                    const selection = $getSelection();
                    if ($isRangeSelection(selection)) {
                         const imageNode = $createImageNode(image.signed_url, image.file_name);
...
function PasteImagePlugin({ taskId, onUploadStart, onUploadEnd }: PasteImagePluginProps) {
     const [editor] = useLexicalComposerContext();
          ...
          const loadContent = () => {
               editor.update(() => {
                    try {
                         ...
                         console.log('📦 Valid nodes to append:', validNodes.length);
...
export default function TaskDescription({ value, onChange, taskId, onImageClick }: TaskDescriptionProps) {
     ...
     console.log('🔴 TaskDescription render:', {
          ...
          initialValue: initialValueRef.current?.substring(0, 100),
          taskId,
     });
