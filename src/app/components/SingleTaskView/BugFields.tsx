     ...
     return (
          <div className="bg-slate-800/40 rounded-xl border border-red-500/20 p-4 space-y-4">
               <div className="flex items-center gap-2 pb-2 border-b border-red-500/20">
                    ...
                    <h3 className="text-xs font-semibold text-red-400 uppercase tracking-wider">{t('bugFields.bugDetails')}</h3>
               </div>
               ...
               <div>
                    <label className="flex items-center gap-2 text-xs text-slate-400 mb-2">
                         ...
                         <span>{t('bugFields.bugLink')}</span>
                         <span className="text-red-400">{t('bugFields.required')}</span>
                    </label>
                    <input
                         type="url"
                         value={bugUrl}
               ...
               <div>
                    <label className="flex items-center gap-2 text-xs text-slate-400 mb-2">
                         <span>{t('bugFields.scenario')}</span>
                    </label>
                    <textarea
                         value={bugScenario}
                         onChange={(e) => onBugScenarioChange(e.target.value)}
                         placeholder={t('bugFields.scenarioPlaceholder')}
                         rows={3}
               ...
               <div className="flex items-center gap-2 bg-slate-700/30 rounded-lg px-3 py-2.5 border border-slate-600/30">
                    ...
                    <p className="text-xs text-slate-400">
                         <Trans i18nKey="bugFields.screenshotHint" components={{ 1: <span className="text-slate-300 font-medium" /> }} />
                    </p>
               </div>
          </div>
     );
