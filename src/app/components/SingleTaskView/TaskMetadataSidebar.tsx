...
const getDisplayData = (
     ...
     if (!user) {
          return { name: unknownLabel, image: '' };
     }
               ...
               <div className="p-4 space-y-4">
                    ...
                    <div>
                         <div className="flex items-center justify-between mb-2">
                              ...
                              {assignees.length > 0 && (
                                   <span className="text-xs bg-brand-500/20 text-brand-300 px-2 py-0.5 rounded-full">{assignees.length}</span>
                              )}
                         </div>
                         {assignees.length > 0 ? (
                                   ...
                                   {assignees.map((assignee) => {
                                        ...
                                        return (
                                             <div key={assignee.id} className="flex items-center gap-2.5 bg-slate-700/30 p-2 rounded-lg border border-slate-700/30 hover:bg-slate-700/50 transition-colors">
                                                  <Avatar src={image} alt={name} size={28} className="ring-2 ring-brand-400/20" />
                              ...
                              <div className="flex items-center justify-center py-3 px-3 bg-slate-700/20 rounded-lg border border-dashed border-slate-600/30">
                                   <span className="text-slate-500 text-xs">{t('taskMeta.noAssigned')}</span>
                              </div>
                         )}
                    </div>
                    ...
                    <div>
                         <div className="flex items-center gap-2 mb-2">
                              ...
                              <h3 className="text-xs font-semibold text-slate-500 uppercase tracking-wider">{t('taskMeta.author')}</h3>
                         </div>
                         {task.creator ? (
                              <div className="flex items-center gap-2.5 bg-slate-700/30 p-2.5 rounded-lg border border-slate-700/30">
                                   {(() => {
                                        ...
                                        return (
                                             <>
                                                  <Avatar src={image} alt={name} size={32} className="ring-2 ring-brand-400/20" />
                                                  <div className="flex flex-col min-w-0 flex-1">
                                                       <span className="text-white text-sm font-medium truncate">{name}</span>
                              ...
                              <div className="text-slate-500 text-xs bg-slate-700/20 p-2.5 rounded-lg">{t('common.unknown')}</div>
                         )}
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-2">
                         <div className="bg-slate-700/30 p-3 rounded-lg border border-slate-700/30">
                              <div className="flex items-center gap-1.5 mb-1">
                                   ...
                                   <h3 className="text-[10px] font-semibold text-slate-500 uppercase">{t('taskMeta.createdAt')}</h3>
                              </div>
                         ...
                         <div className="bg-slate-700/30 p-3 rounded-lg border border-slate-700/30">
                              <div className="flex items-center gap-1.5 mb-1">
                                   ...
                                   <h3 className="text-[10px] font-semibold text-slate-500 uppercase">{t('taskMeta.updatedAt')}</h3>
                              </div>
                    ...
                    <div className="bg-slate-700/30 p-3 rounded-lg border border-slate-700/30">
                         <div className="flex items-center gap-1.5 mb-1">
                              ...
                              <h3 className="text-[10px] font-semibold text-slate-500 uppercase">{t('taskMeta.column')}</h3>
                         </div>
                    ...
                    {/* Task Type */}
                    <div>
                         <h3 className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-2">{t('taskMeta.type')}</h3>
                         {isSubtask ? (
                              <div className="space-y-2">
                                   ...
                                   {task.parent_id && onOpenTask && (
                                        <button
                                             ...
                                             className="w-full flex items-center justify-center gap-2 px-3 py-2.5 bg-brand-500/15 hover:bg-brand-500/25 text-brand-300 rounded-lg text-xs font-medium transition-all duration-200 border border-brand-500/30"
                                        >
                                             <FaLayerGroup className="w-3.5 h-3.5" />
                                             <span>{t('taskMeta.viewStory')}</span>
                                        </button>
                                   )}
                              </div>
                         ) : isStory ? (
                              <div className="inline-flex items-center gap-2 px-3 py-1.5 bg-brand-500/15 text-brand-300 rounded-lg text-xs font-semibold border border-brand-500/30">
                                   <FaLayerGroup className="w-3.5 h-3.5" />
                                   {t('taskMeta.story')}
                              </div>
                         ) : (
                    ...
                    {duration !== null && (
                         <div className="bg-brand-500/10 p-3 rounded-lg border border-brand-500/20">
                              <div className="flex items-center gap-1.5 mb-1">
                                   ...
                                   <h3 className="text-[10px] font-semibold text-slate-500 uppercase">{t('taskMeta.duration')}</h3>
                              </div>
                              <div className="text-lg font-bold text-white">
                                   {duration} <span className="text-xs font-normal text-slate-400">{t('common.day', { count: duration })}</span>
                              </div>
                         </div>
                    )}
                    ...
                    <div className="pt-3 border-t border-slate-700/30">
                         <button
                              ...
                              className={`
                                   ...
                                   ${
                                        task.is_recurring
                                             ? 'bg-brand-500/15 border border-brand-500/30 hover:bg-brand-500/25'
                                             : 'bg-slate-700/30 border border-slate-700/30 hover:bg-slate-700/50 hover:border-brand-500/30'
                                   }
                              `}
                         >
                              <div className={`p-2 rounded-lg transition-all duration-200 ${task.is_recurring ? 'bg-brand-500/20' : 'bg-slate-600/30 group-hover/recurring:bg-brand-500/15'}`}>
                                   <FaRedo className={`w-4 h-4 transition-all duration-200 ${task.is_recurring ? 'text-brand-300' : 'text-slate-400 group-hover/recurring:text-brand-400'}`} />
                              </div>
                              <div className="text-left flex-1">
                                   <span className="font-medium block text-white text-sm">{task.is_recurring ? t('taskMeta.recurring') : t('taskMeta.setRecurrence')}</span>
                                   {task.is_recurring && task.recurrence_interval && task.recurrence_type ? (
                                        <span className="text-xs text-brand-300 block mt-0.5">
                                             {t('taskMeta.recurrencePattern', {
                                                  interval: task.recurrence_interval,
                                                  unit: task.recurrence_type === 'daily' ? t('recurring.day') : task.recurrence_type === 'weekly' ? t('recurring.week') : task.recurrence_type === 'monthly' ? t('recurring.month') : t('recurring.year'),
                                             })}
                                        </span>
                                   ) : (
                                        <span className="text-xs text-slate-500 block mt-0.5">{t('taskMeta.clickToConfigure')}</span>
                                   )}
                              </div>
                         </button>
                    </div>
