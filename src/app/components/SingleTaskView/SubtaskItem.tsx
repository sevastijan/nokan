  ...
  return (
    ...
    >
      ...
      {dragHandleProps && (
        <div
          {...dragHandleProps}
          className="cursor-grab text-slate-500 hover:text-slate-400 opacity-0 group-hover:opacity-100 transition-opacity"
        >
      ...
      {assignees.length > 0 && (
        <div className="flex -space-x-1">
          {assignees.slice(0, 2).map((user) => (
            <div
              key={user.id}
              className="w-5 h-5 rounded-full bg-slate-600 border border-slate-700 flex items-center justify-center overflow-hidden"
