...
const COLOR_MAP: Record<string, { border: string; ring: string; bg: string; bgActive: string; text: string; textActive: string }> = {
  blue: { border: "border-brand-500/50", ring: "ring-brand-500/50", bg: "bg-brand-500/20", bgActive: "bg-brand-500/15", text: "text-brand-400", textActive: "text-brand-300" },
  purple: { border: "border-brand-500/50", ring: "ring-brand-500/50", bg: "bg-brand-500/20", bgActive: "bg-brand-500/15", text: "text-brand-400", textActive: "text-brand-300" },
...
const TaskTypeSelector: React.FC<TaskTypeSelectorProps> = ({
  ...
  const colors = COLOR_MAP[selectedOption.color] || COLOR_MAP.blue;
    ...
    <div className="relative w-full">
      <button
        ...
        className={`
          ...
          ${isOpen ? `${colors.border} ring-2 ${colors.ring} bg-slate-700/70` : "border-slate-600/50 hover:border-slate-500"}
        `}
      >
        ...
        {isOpen && !disabled && (
          ...
          >
            {TASK_TYPES.map((type) => {
              ...
              const tc = COLOR_MAP[type.color] || COLOR_MAP.blue;
              return (
                <button
                  key={type.value}
                  type="button"
                  ...
                  className={`
                    ...
                    ${isSelected ? tc.bgActive : "hover:bg-slate-700/50"}
                  `}
                >
                  <div className={`p-2 rounded-lg ${tc.bg}`}>
                    <span className={isSelected ? tc.text : "text-slate-400"}>
                      {type.icon}
                    </span>
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className={`text-sm font-medium ${isSelected ? tc.textActive : "text-slate-200"}`}>
                      {type.label}
                    </div>
