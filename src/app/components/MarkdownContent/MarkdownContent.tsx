...
const Mention = ({ name }: { name: string }) => (
     <span className="inline-flex items-center rounded-md bg-blue-100 px-2 py-1 text-xs font-semibold text-blue-800 ring-1 ring-inset ring-blue-700/10">@{name}</span>
);

const MarkdownContent = ({ content, onImageClick }: MarkdownContentProps) => {
     return (
          <div className="prose prose-sm max-w-none prose-invert prose-headings:text-gray-100 prose-p:text-gray-300 prose-a:text-blue-400 prose-a:underline hover:prose-a:text-blue-300 prose-strong:text-gray-100 prose-em:text-gray-200 prose-code:text-blue-300 prose-code:bg-gray-800 prose-code:px-1 prose-code:rounded prose-pre:bg-gray-900 prose-pre:p-0 prose-blockquote:border-l-blue-500 prose-blockquote:text-gray-400 prose-li:text-gray-300 prose-table:text-gray-300">
               <ReactMarkdown
                    ...
                    components={{
                         ...
                         a: (props) => (
                              <a href={props.href || '#'} target="_blank" rel="noopener noreferrer" className="text-blue-400 underline hover:text-blue-300 transition">
                                   {props.children}
                              </a>
                         ),
                         ...
                         blockquote: ({ children }) => <blockquote className="border-l-4 border-blue-500 pl-4 italic my-4">{renderWithMentions(children)}</blockquote>,
